...
def generate(texts, input_encoding_dict, model, max_input_length, max_output_length, beam_size, max_beams,
...
def infer(texts, model, params, beam_size=3, max_beams=3, min_cut_off_len=10, cut_off_ratio=1.5):
    ...
    all_decoder_outputs = generate(texts, input_encoding_dict, model, max_input_length, max_output_length, beam_size,
...
def convert_data(input_data, output_data, params):
...
def build_model(params_path='test/params', enc_lstm_units=128, unroll=True, use_gru=False, optimizer='adam',
    ...
    if not use_gru:
        encoder = Bidirectional(LSTM(enc_lstm_units, return_sequences=True, return_state=True, unroll=unroll),
        ...
        encoder = Bidirectional(GRU(enc_lstm_units, return_sequences=True, return_state=True, unroll=unroll),
                                merge_mode='concat')(encoder)
...
if __name__ == '__main__':
    ...
    build_params(input_data=input_data, output_data=output_data, params_path='drive/My Drive/deepcorrect/params',
                 max_lenghts=(300, 300))
    ...
    checkpoint = ModelCheckpoint('drive/My Drive/deepcorrect/checkpoint', monitor='val_acc', verbose=1,
                                 save_best_only=True, mode='max')
    callbacks_list = [checkpoint]

    model.fit(input_train_data, output_train_data, validation_data=(input_valid_data, output_valid_data),
              batch_size=128, epochs=20, callbacks=callbacks_list)
