...
class APIAdapter:
    ...
    def get_score_reports(self, topic, text):
        ...
        overall_score = (coherence_score + grammar_score + vocab_score['predicted_score']) / 3
        overall_score = round(overall_score, 1)
        sample = data_collector.get_sample_topic(topic)
        score_report = dict(
            vocab_score=vocab_score['predicted_score'],
            coherence_score=coherence_score,
            grammar_score=grammar_score,
            overall_score=overall_score,
            grammar=grammar,
            vocab_recommend=vocab_score['vocab_recommend'],
            sample=sample
        )
